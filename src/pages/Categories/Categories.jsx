...
function Categories() {
  ...
  // Nav items (shared between bottom nav on mobile and menu drawer on desktop)
  const navItems = [
    { label: 'Home',       icon: <FiHome size={24} />,        tab: 0 },
    { label: 'Categories', icon: <FiGrid size={24} />,        tab: 1 },
    { label: 'Gifts',      icon: <FiGift size={24} />,        tab: 2 },
    { label: 'Orders',     icon: <FiShoppingBag size={24} />, tab: 3 },
    { label: 'Profile',    icon: <FiUser size={24} />,        tab: 4 },
  ];

  return (
    <div className="scrollable-content no-scrollbar">
      {/* ── Slide-out Menu Drawer (hamburger menu) ── */}
      {/* Overlay */}
      <div
      ...
      <div className={`menu-drawer ${menuOpen ? 'open' : ''}`}>
        ...
        <nav className="menu-drawer-nav">
          {navItems.map((item) => (
            <button
              key={item.tab}
              className={`menu-nav-item ${activeTab === item.tab ? 'active' : ''}`}
              onClick={() => {
                setActiveTab(item.tab);
                closeMenu();
                handleAction(`Tab ${item.label}`);
              }}
            >
              {item.icon}
              {item.label}
      ...
      <header className="app-header">
        <button className="icon-btn" onClick={() => setMenuOpen(true)}>
          <FiMenu size={26} color="#1E1E2C" />
        </button>
        <h1 className="app-title">Categories</h1>
        <div className="header-actions">
          <button className="icon-btn" onClick={() => handleAction('Search opened')}>
            <FiSearch size={24} color="#1E1E2C" />
          </button>
          <button className="icon-btn" onClick={() => handleAction('Cart opened')}>
            <FiShoppingCart size={24} color="#1E1E2C" />
          ...
          {categoryDisplay.map((item, index) => (
            <div
              ...
              onClick={() => {
                ...
                handleAction(`Selected ${item.title}`);
              }}
            >
      ...
      <div className={`occasions-row no-scrollbar ${expandOccasions ? 'expanded' : ''}`}>
        {[
          ...
          <div key={index} className="occasion-item" onClick={() => handleAction(`Selected ${item.label}`)}>
      ...
      <div className="popular-list">
        {[
          ...
          <div key={index} className="popular-card" style={{ backgroundColor: item.bgColor }} onClick={() => handleAction(`Shopping ${item.title}!`)}>
      ...
      <nav className="bottom-nav">
        {navItems.map((item, index) => (
          <button
            ...
            onClick={() => { setActiveTab(item.tab); handleAction(`Tab ${item.label}`); }}
          >
